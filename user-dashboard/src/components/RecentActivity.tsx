...
import { CheckCircleOutlined, PlusCircleOutlined, FormOutlined, PlayCircleOutlined } from '@ant-design/icons';
...
const RecentActivity = () => {
    ...
    if (loading) {
        return <Card title="Recent Activity"><Spin /></Card>;
    }
                    ...
                    return (
                        <List.Item>
                            <List.Item.Meta
                                ...
                                title={<Text>{textMap[item.type] ? textMap[item.type](metadata) : 'Unknown activity'}</Text>}
                                description={<Text type="secondary">{moment(item.timestamp).fromNow()}</Text>}
                            />
                        </List.Item>
                    )
                }}
