...
const DraggableGameCard = ({ game, onTitleClick }: { game: UserGame; onTitleClick: (game: UserGame) => void }) => {
...
const DroppableColumn = ({ id, title, children }: { id: string; title: string; children: React.ReactNode }) => {
    ...
    return (
        <Col xs={24} sm={12} xl={4}>
            <div 
                ref={setNodeRef} 
                className="bg-neutral-200 p-4 rounded-lg h-full"
            >
                <Row justify="space-between" align="middle" style={{ marginBottom: '16px' }}>
                    <Title level={5} style={{ margin: 0, textTransform: 'uppercase', color: '#6B778C' }}>{title}</Title>
...
const BoardView: React.FC = () => {
    ...
    const handleGameUpdate = async (values: Partial<UserGame>) => {
        ...
        try {
            ...
            setGames(prevGames => 
                prevGames.map(g => (g.$id === editingGame.$id ? { ...g, ...updatedGame.data } : g))
            );
            handleModalClose();
        } catch (error) {
    ...
    const handleDragEnd = async (event: DragEndEvent) => {
        ...
        if (over && active.id !== over.id) {
            ...
            const updatedGames = games.map(g => g.$id === gameId ? { ...g, status: newStatus } : g);
        ...
        <DndContext onDragEnd={handleDragEnd}>
            <div>
                <Row justify="space-between" align="middle" style={{ marginBottom: '24px' }}>
                    <Title level={2} style={{ margin: 0 }}>Kanban Board</Title>
                    <div>
                    ...
                    {statusColumns.map(col => (
                        <DroppableColumn key={col.id} id={col.id} title={col.title}>
                            {games
                                ...
                                .map(game => (
                                    <DraggableGameCard key={game.$id} game={game} onTitleClick={handleTitleClick} />
                                ))
                            }
                        </DroppableColumn>
                    ))}
