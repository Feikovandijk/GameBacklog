...
import { GameStats, OwnedGame, PlayerAchievement, User } from '../types/steam.types';
...
async function getPlayerAchievements(
  ...
  try {
    ...
    // console.log(`Could not fetch achievements for appid ${appId}:`, error);
    return [];
  }
}
...
async function getUserBacklog(userId: string): Promise<Map<number, UserGameRecord>> {
  const { data, error } = await supabase
    .from('user_games')
    .select('id, steam_appid, hours_played, playtime_2weeks')
    .eq('user_id', userId);

  if (error) {
    console.error(`Error fetching user backlog for user ${userId}:`, error);
    return new Map();
  }

  const backlog = new Map<number, UserGameRecord>();
  data.forEach(doc => backlog.set(Number(doc.steam_appid), doc as UserGameRecord));
  return backlog;
}
 ...
 */
export async function syncUserWithSteam(user: User): Promise<void> {
  console.log(
  ...
  for (const game of ownedGames) {
    ...
    let userGameId: string | undefined = existingGame?.id;
    ...
    if (existingGame) {
      ...
      try {
        ...
        if (masterGameResponse) {
          ...
          if (insertError) {
            console.error(
              `Error inserting new game ${game.name} to backlog:`,
              insertError,
            );
          } else if (newUserGame) {
          ...
          if (error) {
            ...
            console.warn(
              `Master game not found for ${game.name} (appid=${game.appid}); skipping insert.`,
            );
          }
        }
      } catch (error) {
